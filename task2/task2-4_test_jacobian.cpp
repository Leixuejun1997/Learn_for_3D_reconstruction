              ...
              double *point_x_ptr, double *point_y_ptr)
{
    ...
    const double *R = cam.rotation;      //旋转矩阵
    const double *t = cam.translation;   //平移向量
    const double *X = point.pos;         // 3D点坐标
    const double k0 = cam.distortion[0]; //径向畸变系数k0
    const double k1 = cam.distortion[1]; //径向畸变系数k1
    //世界坐标系转相机坐标系
    ...
    // 相机焦距的偏导数
    cam_x_ptr[0] = distort * x; // P_u/P_f=d(k0,k1,r^2)*x
    cam_y_ptr[0] = distort * y; // P_v/P_f=d(k0,k1,r^2)*y
