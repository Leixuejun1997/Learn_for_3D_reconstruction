...
// Descriptor存储着特征点的x，y，scale，方向，以及128维的特征描述子
void convert_sift_discriptors(features::Sift::Descriptors const &sift_descrs,
                              util::AlignedMemory<math::Vec128f, 16> *aligned_descr) //数据类型转换
{
    aligned_descr->resize(sift_descrs.size()); // 128维特征描述子
...
{
    std::string exif_str;
    core::image::load_jpg_file(img_name.c_str(), &exif_str);                                          //输入图片
    core::image::ExifInfo exif = core::image::exif_extract(exif_str.c_str(), exif_str.size(), false); // c_str()把string转换为C中的字符串
    sfm::FocalLengthEstimate fl = sfm::extract_focal_length(exif);                                    //提取exif中的焦距信息
    std::cout << "Focal length: " << fl.first << " " << fl.second << std::endl;                       //输出两个焦距信息
    return fl.first;
}

sfm::Correspondences2D2D sift_feature_matching(sfm::FeatureSet &feat1, sfm::FeatureSet &feat2) //特征提取，特征匹配，去除匹配不一致的点，ransac八点法求基础矩阵
{
    ...
    // 特征匹配参数设置
    features::Matching::Options matching_opts; // Options定义基本的参数：描述子的长度，距离阈值，距离之比的阈值
    matching_opts.descriptor_length = 128;
    matching_opts.distance_threshold = 1.0f;
    matching_opts.lowe_ratio_threshold = 0.8f; //最好与次好匹配的距离之比满足的阈值，sift是0.8，surf是0.7,越接近1，则匹配结果越不明确

    // 特征匹配
    features::Matching::Result matching_result; //储存匹配结果
    features::Matching::twoway_match(matching_opts,
                                     aligned_descrs1.data()->begin(),
                                     feat1.sift_descriptors.size(),
                                     aligned_descrs2.data()->begin(),
                                     feat2.sift_descriptors.size(),
                                     &matching_result); //双向匹配，并把匹配结果保存在matching_result中
    // 去除不一致的匹配对
    features::Matching::remove_inconsistent_matches(&matching_result);
    int n_consitent_matches = features::Matching::count_consistent_matches(matching_result); //匹配一致的点的数量
    ...
    sfm::Correspondences2D2D corr_all; //存储全部匹配的特征点对，把所有的匹配点对储存到sfm::Correspondences2D2D中
    ...
    for (int i = 0; i < m12.size(); i++)
    {
        ...
        c2d2d.p2[0] = feat2.positions[m12[i]][0]; //匹配点一一对应存储
        c2d2d.p2[1] = feat2.positions[m12[i]][1];

        corr_all.push_back(c2d2d);
    }
    /* RANSAC 估计本征矩阵, 并对特征匹配对进行筛选*/
    sfm::RansacFundamental::Options ransac_fundamental_opts; // Options定义基本的参数：最大迭代次数，确定内点的阈值(默认为：00015)
    ...
    ransac_fundamental.estimate(corr_all, &ransac_fundamental_result); //相当于给了大于8对的匹配点对，然后随机挑选8对匹配点，在使用八点法进行基础矩阵的求解
    // 根据估计的Fundamental矩阵对特征匹配对进行筛选
    sfm::Correspondences2D2D corr_f; //储存内点
...
bool calc_cam_poses(sfm::Correspondences2D2D const &matches, sfm::CameraPose *pose1, sfm::CameraPose *pose2, float f1, float f2)
{
    ...
    sfm::fundamental_least_squares(matches, &F); //最小二乘法求基础矩阵（将所有的匹配点组成一个大矩阵n*9的矩阵，再对这个矩阵进行分解得到最终的基础矩阵）
    sfm::enforce_essential_constraints(&F);      //本质矩阵分解
    ...
    // todo 从基础矩阵中计算本质矩阵
    sfm::EssentialMatrix E = pose2->K.transpose() * F * pose1->K; // E=K2^T*F*K1
    ...
    // 利用匹配点对从4个相机姿态中找到最合适的(利用姿态对匹配点进行三角化得到三维点，计算三维点的相机坐标，
    // 在两个相机的中的z坐标必须为正
...
int main(int argc, char *argv[])
{
    ...
    std::ofstream out("/home/lei/git_for_homework/data/test_ba.txt"); //将结果保存
    assert(out.is_open());
