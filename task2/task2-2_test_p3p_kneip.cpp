...
void solve_quartic_roots(math::Vec5d const &factors, math::Vec4d *real_roots)
{
    ...
    std::complex<double> P(-alpha2 / 12.0 - gamma, 0.0); //complex(实部，虚部)
    std::complex<double> Q(-alpha3 / 108.0 + alpha * gamma / 3.0 - beta2 / 8.0, 0.0);
    std::complex<double> R = -Q / 2.0 + std::sqrt(Q * Q / 4.0 + P * P * P / 27.0); //sqrt()开平方函数
    ...
    std::vector<math::Matrix<double, 3, 4>> *solutions)
{
    ...
    //如果三个点是共线的则无解
    double const colinear_threshold = 1e-10;                         //判断共线的阈值
    if ((p2 - p1).cross(p3 - p1).square_norm() < colinear_threshold) //通过求p2p1叉乘p3p1所得向量的二范数的大小来判断是否共线，
                                                                     //当二范数小于共线阈值之后定为三点共线
    {
        solutions->clear();
        return;
    ...
    //归一化方向
    double const normalize_epsilon = 1e-10;//归一化epsilon
    if (!MATH_EPSILON_EQ(f1.square_norm(), 1.0, normalize_epsilon))//1-1e-10<=||f1||2<=1+1e-10,判断一下是否归一化，未归一化则归一化
        f1.normalize();
    ...
    math::Matrix3d T;
    {
        ...
        f3 = T * f3;//这里的f3已经转到中间相机坐标系
    }

    /* Change camera frame and point order if f3[2] > 0. */
    //如果f3在中间相机坐标系的z>0时交换p1，p2，交换f1，f2
    if (f3[2] > 0.0)
    {
        std::swap(p1, p2);
        std::swap(f1, f2);
    ...
    p3 = N * (p3 - p1);//P3点在中间世界坐标系的坐标

    /* Extraction of known parameters. */
    double d_12 = (p2 - p1).norm();//d_12的长度
    double f_1 = f3[0] / f3[2];//φ1
    double f_2 = f3[1] / f3[2];//φ2
    double p_1 = p3[0];//p1
    double p_2 = p3[1];//p2

    double cos_beta = f1.dot(f2);// cosβ=f1*f2,β为f1和f2的夹角
    double b = 1.0 / (1.0 - MATH_POW2(cos_beta)) - 1;//论文中的b^2

    if (cos_beta < 0.0)//cosβ<0,则b取负值,f1,f2之间的夹角在(0,π)
        b = -std::sqrt(b);
    else
        b = std::sqrt(b);//cosβ>0,则b取正值

    /* Temporary pre-computed variables. */
    double f_1_pw2 = MATH_POW2(f_1);//φ1^2
    double f_2_pw2 = MATH_POW2(f_2);//φ2^2
    double p_1_pw2 = MATH_POW2(p_1);//p1^2
    double p_1_pw3 = p_1_pw2 * p_1;//p1^3
    double p_1_pw4 = p_1_pw3 * p_1;//p1^4
    double p_2_pw2 = MATH_POW2(p_2);//p2^2
    double p_2_pw3 = p_2_pw2 * p_2;//p2^3
    double p_2_pw4 = p_2_pw3 * p_2;//p2^4
    double d_12_pw2 = MATH_POW2(d_12);//d12^2
    double b_pw2 = MATH_POW2(b);//b^2

    /* Factors of the 4th degree polynomial. */
    math::Vec5d factors;
    
    factors[0] = -f_2_pw2 * p_2_pw4 - p_2_pw4 * f_1_pw2 - p_2_pw4;//a4

    factors[1] = 2.0 * p_2_pw3 * d_12 * b + 2.0 * f_2_pw2 * p_2_pw3 * d_12 * b - 2.0 * f_2 * p_2_pw3 * f_1 * d_12;//a3

    factors[2] = -f_2_pw2 * p_2_pw2 * p_1_pw2 - f_2_pw2 * p_2_pw2 * d_12_pw2 * b_pw2 - f_2_pw2 * p_2_pw2 * d_12_pw2 + f_2_pw2 * p_2_pw4 + p_2_pw4 * f_1_pw2 + 2.0 * p_1 * p_2_pw2 * d_12 + 2.0 * f_1 * f_2 * p_1 * p_2_pw2 * d_12 * b - p_2_pw2 * p_1_pw2 * f_1_pw2 + 2.0 * p_1 * p_2_pw2 * f_2_pw2 * d_12 - p_2_pw2 * d_12_pw2 * b_pw2 - 2.0 * p_1_pw2 * p_2_pw2;//a2

    factors[3] = 2.0 * p_1_pw2 * p_2 * d_12 * b + 2.0 * f_2 * p_2_pw3 * f_1 * d_12 - 2.0 * f_2_pw2 * p_2_pw3 * d_12 * b - 2.0 * p_1 * p_2 * d_12_pw2 * b;//a1

    factors[4] = -2.0 * f_2 * p_2_pw2 * f_1 * p_1 * d_12 * b + f_2_pw2 * p_2_pw2 * d_12_pw2 + 2.0 * p_1_pw3 * d_12 - p_1_pw2 * d_12_pw2 + f_2_pw2 * p_2_pw2 * p_1_pw2 - p_1_pw4 - 2.0 * f_2_pw2 * p_2_pw2 * p_1 * d_12 + p_2_pw2 * f_1_pw2 * p_1_pw2 + f_2_pw2 * p_2_pw2 * d_12_pw2 * b_pw2;//a0

    /* Solve for the roots of the polynomial. */
    math::Vec4d real_roots;
    solve_quartic_roots(factors, &real_roots);//求解出a4,a3,a2,a1,a0,解一元四次方程
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        double cos_theta = real_roots[i];//cosθ
        double sin_theta = std::sqrt(1.0 - MATH_POW2(real_roots[i]));//sinθ=sqrt(1-cosθ^2)
        double sin_alpha = std::sqrt(1.0 / (MATH_POW2(cot_alpha) + 1));//sinα=sqrt(1/(cotα)^2+1)
        double cos_alpha = std::sqrt(1.0 - MATH_POW2(sin_alpha));//cosα=sqrt(1-sinα^2)

        if (cot_alpha < 0.0)//cotα<0
            cos_alpha = -cos_alpha;//cosα=-cosα

        math::Vec3d C(
            d_12 * cos_alpha * (sin_alpha * b + cos_alpha),
            cos_theta * d_12 * sin_alpha * (sin_alpha * b + cos_alpha),
            sin_theta * d_12 * sin_alpha * (sin_alpha * b + cos_alpha));//相机中心在中间世界坐标系中的表示

        C = p1 + N.transposed() * C;//相机中心在世界坐标系的表示

        math::Matrix3d R;//Q(α; θ)，η到τ的转换矩阵
        R[0] = -cos_alpha;
        ...
        R = N.transposed().mult(R.transposed()).mult(T);//R=N^T*Q^T*T

        /* Convert camera position and cam-to-world rotation to pose. */
        R = R.transposed();//世界坐标系到相机坐标系
        C = -R * C;//将相机中心转回相机坐标系

        solutions->at(i) = R.hstack(C);//将R和C组合
    }
}
...
void compute_p3p(sfm::Correspondences2D3D const &corresp,
                 ...
                 PutativePoses *poses)//存放3*4的矩阵的容器 //Correspondences2D3D存放一个3D点和其对应的二维点
{

    if (corresp.size() < 3)//要三对3D-2D点才能kenip
        throw std::invalid_argument("At least 3 correspondences required");

    /* Draw 3 unique random numbers. */
    std::set<int> result;
    while (result.size() < 3)
        result.insert(util::system::rand_int() % corresp.size());//随机1-3的随机数
...
int main(int argc, char *argv[])
{
    ...
    // 计算相机坐标系中对应的摄线
    math::Vec3d f1 = inv_k_matrix.mult(math::Vec3d(uv1[0], uv1[1], 1.0));//K^(-1)*uv=xy将像素坐标系转到相机坐标系
    ...
    for (std::size_t j = 0; j < solutions.size(); ++j)
    {
        ...
        math::Vec3d p2d = k_matrix * (solutions[j] * p3d);//K*R*Pw=像素坐标系
