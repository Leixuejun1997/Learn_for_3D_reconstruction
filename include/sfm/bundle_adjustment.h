...
public:
    ...
    struct Options
    {
        ...
        bool verbose_output;
        BAMode bundle_mode;    //枚举变量,默认为BA_CAMERAS_AND_POINTS=1|2
        bool fixed_intrinsics; // 固定内参数，不进行优化
        // bool shared_intrinsics;
        int lm_max_iterations; // LM的最大迭代次数，默认为50
        int lm_min_iterations; // LM的最小迭代次数，默认为0

        double lm_delta_threshold;         // delta的阈值，默认1e-4
        double lm_mse_threshold;           // lm均方误差的阈值，默认1e-8
        LinearSolver::Options linear_opts; //共轭梯度法求解delta_x,delta_y的参数设置
    };
    ...
    struct Status
    {
        ...
        double initial_mse;                 //初始均方误差
        double final_mse;                   //最终均方误差
        int num_lm_iterations;              // lm迭代次数
        int num_lm_successful_iterations;   // lm成功的迭代次数
        int num_lm_unsuccessful_iterations; // lm不成功的迭代次数
        int num_cg_iterations;              //共轭梯度法的迭代次数
        std::size_t runtime_ms;             //计算时间
    };
...
inline BundleAdjustment::Options::Options(void)
    : verbose_output(false), bundle_mode(BA_CAMERAS_AND_POINTS), lm_max_iterations(50), lm_min_iterations(0), lm_delta_threshold(1e-4), lm_mse_threshold(1e-8)
{
}

inline BundleAdjustment::Status::Status(void)
    : initial_mse(0.0), final_mse(0.0), num_lm_iterations(0), num_lm_successful_iterations(0), num_lm_unsuccessful_iterations(0), num_cg_iterations(0)
{
}

inline BundleAdjustment::BundleAdjustment(Options const &options)
    : opts(options), log(options.verbose_output
                             ? util::Logging::LOG_DEBUG
                             : util::Logging::LOG_INFO),
      cameras(nullptr), points(nullptr), observations(nullptr), num_cam_params(options.fixed_intrinsics ? 6 : 9)
{
