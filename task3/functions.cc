...
std::string
remove_file_extension (std::string const& filename)//去除文件的扩展名
{
    std::size_t pos = filename.find_last_of('.');//从后往前寻找字符“.”，并返回位置
...
bool
has_jpeg_extension (std::string const& filename)//判断文件是否有扩展名
{
    std::string lcfname(util::string::lowercase(filename));//全是小写的字符串
...
add_exif_to_view (core::View::Ptr view, std::string const& exif)
{
    ...
    core::ByteImage::Ptr exif_image = core::ByteImage::create(exif.size(), 1, 1);//定义一个ByteImage
    std::copy(exif.begin(), exif.end(), exif_image->begin());//将exif中的信息复制到exif_image中
    view->set_blob(exif_image, "exif");//命名为exif
}
