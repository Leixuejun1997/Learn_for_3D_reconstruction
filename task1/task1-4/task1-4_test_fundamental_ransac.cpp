 ...
 *       需要的采样次数 M = ——————————————
 *                          log(1-p^K)
 * Example: For p = 50%, z = 99%, n（也就是K） = 8: M = log(0.001) / log(0.99609) = 1176.
                           ...
                           double z = 0.99)
{
    ...
    /** Reference*/
    double prob_all_good = math::fastpow(p, K); //fastpow()快速幂函数：p^k
...
std::vector<int> find_inliers(sfm::Correspondences2D2D const &matches, FundamentalMatrix const &F, const double &thresh)
{
    ...
    return inliers; //把内点的序号储存在vector内
}
...
int main(int argc, char *argv[])
{
    ...
    assert(in.is_open());//如果它的条件返回错误，则终止程序执行，如果文件打开失败，则终止程序

    std::string line, word;
    int n_line = 0;
    while (getline(in, line))//getline(in，line)将in中的逐行读到line变量中
    {

        std::stringstream stream(line);//将读到的line流入stream
        if (n_line == 0)
        {
            int n_corrs = 0;
            stream >> n_corrs;//将stream的转为整型
    ...
    for (int i = 0; i < n_iterations; i++)
    {
        ...
        for (int j = 0; j < 8; j++, iter++)
        {
            ...
            pset1(0, j) = match.p1[0];//第一个匹配点的X
            pset1(1, j) = match.p1[1];//第一个匹配点的Y
            pset1(2, j) = 1.0;

            pset2(0, j) = match.p2[0];//第二个匹配点的X
            pset2(1, j) = match.p2[1];//第二个匹配点的Y
            pset2(2, j) = 1.0;
        }
