...
public:
    struct Options
    {
        ...
        /** Threshold (factor of the median error) for large error tracks. 大误差tracks的阈值(中值误差因子)*/
        double track_error_threshold_factor;
        /** Reprojection error threshold for newly triangulated tracks. 新三角化的tracks的重投影误差阈值*/
        double new_track_error_threshold;
        /** Minimum angle for track triangulation in RAD. RAD中tracks三角化的最小角度*/
        double min_triangulation_angle;
        /** Bundle Adjustment fixed intrinsics. 捆绑调整固定的内参矩阵*/
        bool ba_fixed_intrinsics;
        /** Bundle Adjustment with shared intrinsics. 捆绑调整共享的内参矩阵*/
