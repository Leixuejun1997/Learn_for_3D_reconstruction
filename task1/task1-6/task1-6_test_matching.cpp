...
void feature_set_matching(core::ByteImage::Ptr image1, core::ByteImage::Ptr image2)
{
    ...
    sift_matching_opts.lowe_ratio_threshold = 0.8f;                            //sift的取值0.8,surf取值0.7
    sift_matching_opts.descriptor_length = 128;                                //sift特征描述符为128维,surf的为64
    sift_matching_opts.distance_threshold = std::numeric_limits<float>::max(); //用于获得在当前系统中float类型能取得的最大值

//通过修改这个宏常量，来改变程序是编译#if后面的语句还是#else后面的语句，
//这里DISCRETIZE_DESCRIPTORS通过（0，1）来修改描述符的数据类型是（Vec128us,Vec128f）
#if DISCRETIZE_DESCRIPTORS
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        image1 = core::image::load_file(std::string("./kxm1.jpg"));
        // 图像尺寸减半
