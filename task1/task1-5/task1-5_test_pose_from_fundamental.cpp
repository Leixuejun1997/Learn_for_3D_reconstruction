 ...
 * @return 三维点
 */
math::Vec3d triangulation(math::Vec2d const &p1, math::Vec2d const &p2, math::Matrix3d const &K1, math::Matrix3d const &R1, math::Vec3d const &t1, math::Matrix3d const &K2, math::Matrix3d const &R2, math::Vec3d const &t2)
{
 ...
 * @return
 */
bool is_correct_pose(math::Matrix3d const &R1, math::Vec3d const &t1, math::Matrix3d const &R2, math::Vec3d const &t2)
{
    ...
    math::Vec3d p3d = triangulation(p1, p2, K1, R1, t1, K2, R2, t2); //三角化得到的世界坐标系的点的坐标
    math::Vector<double, 3> x1 = R1 * p3d + t1;                      //世界坐标系转化到相机坐标系，得到相机坐标系下的x1,x2
    math::Vector<double, 3> x2 = R2 * p3d + t2;
    return x1[2] > 0.0f && x2[2] > 0.0f; //判断相机坐标系Zc是否大于0；
}

bool calc_cam_poses(FundamentalMatrix const &F, const double f1, const double f2, math::Matrix3d &R, math::Vec3d &t)
{
    /* 相机内参矩阵 */
    math::Matrix3d K1(0.0), K2(0.0);
    K1(0, 0) = K1(1, 1) = f1;
    ...
    EssentialMatrix E = K2.transpose() * F * K1; //本质矩阵E=K2^t*F*K1
    ...
    math::matrix_svd(E, &U, &S, &V); //对E进行SVD分解，分解出R，t

    // 保证旋转矩阵行列式 det(R) = 1 (instead of -1).
    //一个矩阵是旋转矩阵，当且仅当他是正交矩阵并且它的行列式为1
    ...
    poses[0].first = U * W * V; //R1=U*Rz(PI/2)*V^T 其中Rz=W
    poses[1].first = U * W * V;
    poses[2].first = U * Wt * V; //R2=U*R(^Tz)*V^T 其中R(^Tz)=Wt
    poses[3].first = U * Wt * V;
    poses[0].second = U.col(2);//t为U的最后一列
    ...
    // 判断姿态是否合理
    bool flags[4]; //记录四种不同情况的R，t判断结果
    for (int i = 0; i < 4; i++)
    {
        flags[i] = is_correct_pose(R1, t1, poses[i].first, poses[i].second);
    }
    //找到正确的姿态
    if (flags[0] || flags[1] || flags[2] || flags[3]) //四种只要有一种符合Zc>0的条件，则返回true
    {
        for (int i = 0; i < 4; i++)
        {
            if (!flags[i]) //不符合直接跳过
                continue;
            R = poses[i].first; //将符合条件的赋值给R，t
            t = poses[i].second;
        }
        return true;
    }
